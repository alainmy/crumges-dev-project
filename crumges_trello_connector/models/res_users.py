...
class ResUsers(models.Model):
    ...
    def _get_trello_auth_user(self):
        """Devuelve un usuario con API key y token válidos para Trello.

        La sincronización debe usar una cuenta autenticada aunque el usuario que
        dispara la edición desde Odoo no sea la misma que tiene configurado
        Trello. Sin este fallback, las llamadas a _sync_to_trello() salen sin
        hacer nada porque se usan las credenciales del usuario activo.
        """
        user = self.env.user
    ...
    trello_member_id = fields.Char(
        string='ID Trello', help="ID único del miembro en Trello", index=True)
    ...
    trello_webhook_url = fields.Char(
        string='URL Webhook Trello', help="URL de la Webhook de Trello", index=True)

    def write(self, vals):
        res = super(ResUsers, self).write(vals)
        for user in self:
            success = False
            if not user.trello_webhook_url:
            ...
            if 'trello_sync_enabled' in vals:
                if vals['trello_sync_enabled']:
                    if success:
                        ...
                        raise UserError(
                            "Error al conectar con Trello. Revisa tu API Key y Token.")
                else:
                    user._delete_trello_webhook()
        return res

    def action_trello_webhook(self):
        self._register_trello_webhook()
        return True
    ...
    def _get_or_create_trello_user(self, member_id, fallback_name="Usuario Trello"):
        ...
        user = self.sudo().search(
            [('trello_member_id', '=', member_id)], limit=1)
        if user:
            return user
        ...
        trello_user = self.env.user if self.env.user.trello_token else self.sudo(
        ).search([('trello_token', '!=', False)], limit=1)
        ...
        if trello_user and trello_user.trello_token:
            member_data = trello_user._trello_request(
                'GET', f'/members/{member_id}')
            if isinstance(member_data, dict):
                name = member_data.get('fullName') or member_data.get(
                    'username') or fallback_name
        ...
        partner.message_post(
            body=f"Contacto creado por sincronización automática con Trello (Miembro ID: {member_id})")
    ...
    def _fetch_trello_boards(self):
        self.ensure_one()
        boards = self._trello_request(
            'GET', f'/tokens/{self.trello_token}/member/boards')
        if isinstance(boards, list):
            for board in boards:
                existing = self.env['trello.board'].search(
                    [('trello_id', '=', board.get('id'))], limit=1)
                if not existing:
    ...
    def _webhook_request(self, method, endpoint, params=None):
        self.ensure_one()
        import requests
        ...
        try:
            ...
            logging.getLogger(__name__).error(
                f"Trello Webhook Request Error: {e}")
            return False
    ...
    def _register_trello_webhook(self):
        for user in self:
            ...
            member_data = user._trello_request(
                'GET', f'/tokens/{user.trello_token}/member')
            if not member_data or 'id' not in member_data:
                continue

            member_id = member_data['id']
            # Buscar webhooks existentes para no duplicar
            existing_webhooks = user._trello_request(
                'GET', f'/tokens/{user.trello_token}/webhooks')
            webhook_id = False
            ...
            if not webhook_id:
                ...
                res = user._webhook_request('POST', '/webhooks', params=params)
                if not res:
                    return False
                user.write({'trello_webhook_url': callback_url})
        return True
    ...
    def _process_trello_webhook_event(self, action_data):
        ...
        if action_type == 'addMemberToBoard':
            ...
            if member.get('id') and board_id:
                ...
                if project:
                    t_user = self._get_or_create_trello_user(
                        member['id'], member.get('fullName'))
                    project.message_subscribe(
                        partner_ids=[t_user.partner_id.id])
            return
        ...
        if action_type == 'updateList':
            ...
            if list_id and board_id:
                ...
                if project:
                    relation = self.env['project.trello.stage'].sudo().search([
        ...
        if action_type == 'addMemberToCard':
            ...
            if member.get('id') and card_id:
                task_add = self.env['project.task'].sudo().search(
                    [('trello_card_id', '=', card_id)], limit=1)
                if task_add:
                    t_user = self._get_or_create_trello_user(
                        member['id'], member.get('fullName'))
                    task_add.with_context(trello_webhook_sync=True).write(
                        {'user_ids': [(4, t_user.id)]})
                    task_add.message_subscribe(
                        partner_ids=[t_user.partner_id.id])
            return
        ...
        if action_type == 'removeMemberFromCard':
            ...
            card_id = action_data.get('data', {}).get('card', {}).get('id')
            if member.get('id') and card_id:
                task_remove = self.env['project.task'].sudo().search(
                    [('trello_card_id', '=', card_id)], limit=1)
                if task_remove:
                    t_user = self.sudo().search(
                        [('trello_member_id', '=', member['id'])], limit=1)
                    if t_user:
                        task_remove.with_context(trello_webhook_sync=True).write(
                            {'user_ids': [(3, t_user.id)]})
                        task_remove.message_unsubscribe(
                            partner_ids=[t_user.partner_id.id])
            return
        ...
        if action_type in ('addLabelToCard', 'removeLabelFromCard'):
            task.with_user(user).with_delay(
                channel='root.trello_sync')._sync_to_trello('update')
            return
        ...
        if 'name' in card_data or 'desc' in card_data or 'closed' in card_data or 'dueComplete' in card_data or 'due' in card_data or 'start' in card_data or 'idList' in card_data:
            ...
            if 'due' in card_data:
                vals['date_deadline'] = card_data['due'][:19].replace(
                    'T', ' ') if card_data['due'] else False
            if 'start' in card_data:
                vals['planned_date_begin'] = card_data['start'][:19].replace(
                    'T', ' ') if card_data['start'] else False
            if 'idList' in card_data and 'idList' in action_data.get('data', {}).get('old', {}):
                ...
                if board_id:
                    ...
                    if project:
                        relation = self.env['project.trello.stage'].sudo().search([
                            ('project_id', '=', project.id),
                            ('trello_list_id', '=', card_data['idList']),
                        ], limit=1)
                        stage = relation.stage_id if relation else False
                        if stage:
        ...
        if action_type == 'commentCard':
            ...
            if text:
                ...
                body = Markup(
                    f"<b>{member.get('fullName', 'Usuario de Trello')} comentó:</b><br/>{text}<!--{action_id}-->")

                # Evitar duplicados
                existing = self.env['mail.message'].sudo().search(
                    [('res_id', '=', task.id), ('model', '=', 'project.task'), ('body', 'ilike', action_id)], limit=1)
                if not existing:
                    task.with_context(trello_webhook_sync=True).message_post(
                        body=body, partner_ids=mentioned_partners)
        # Adjuntos
        if action_type == 'addAttachmentToCard':
            ...
            if attachment_data and attachment_data.get('url'):
                ...
                headers = {
                    'Authorization': f'OAuth oauth_consumer_key="{user.trello_api_key}", oauth_token="{user.trello_token}"'}
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        task.with_context(trello_webhook_sync=True).message_post(
                            body=Markup(
                                f"<b>{member.get('fullName', 'Usuario de Trello')} adjuntó:</b> {attachment_data.get('name', '')}<!--{action_id}-->"),
        ...
        if list_after and list_after.get('id'):
            ...
            if board_id:
                ...
                if project:
                    relation = self.env['project.trello.stage'].sudo().search([
                        ('project_id', '=', project.id),
                        ('trello_list_id', '=', list_after['id']),
                    ], limit=1)
                    stage = relation.stage_id if relation else False
                    if stage:
                        task.with_context(trello_webhook_sync=True).write(
                            {'stage_id': stage.id})
